...
void GroundSteeringMessageHandler::setup(cluon::OD4Session& od4) {
    auto on_ground_steering_request = [this](cluon::data::Envelope&& env) {
        ...
        m_ground_steering_request = cluon::extractMessage<opendlv::proxy::GroundSteeringRequest>(std::move(env));
    };
    od4.dataTrigger(opendlv::proxy::GroundSteeringRequest::ID(), on_ground_steering_request);
}
