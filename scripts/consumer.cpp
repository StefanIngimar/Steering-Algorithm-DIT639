 ...
 * */
int display_frame(uint32_t content_length,
                  std::vector<uint8_t>& frame_data_buffer) {
  ...
  try {
    ...
    if (!decoder.open(temp_file, cv::CAP_FFMPEG)) {
      std::cerr << "Failed to open decoder for temporary file " << temp_file
                << std::endl;
      return -1;
    }
...
int main() {
  ...
  if (shm_id == -1) {
    std::cerr << "Error: Failed to get shared memory with key 0x" << std::hex
              << SHM_KEY << ": " << std::strerror(errno) << std::endl;
    return 1;
  }
  ...
  std::cout << "Attached to shared memory with key 0x" << std::hex << SHM_KEY
            << std::dec << std::endl;

  // Cluon shared memory setup
  const int cluon_cid = 253;
  cluon::OD4Session od4(cluon_cid,
                        [](cluon::data::Envelope&& envelope) noexcept {});
  if (!od4.isRunning()) {
    std::cerr << "Failed to start cluon OD4 session with CID " << cluon_cid
              << std::endl;
    shmdt(shm_ptr);
    return 1;
  }
  ...
  while (true) {
    ...
    if (is_frame_available == 0 && should_terminate == 1) {
      std::cout
          << "Termination signal received from python. Exiting the main loop."
          << std::endl;
      break;
    }
    ...
    if (content_length > MAX_H264_FRAME_SIZE) {
      std::cerr << "Received frame is too big: " << content_length << " bytes"
                << std::endl;
      data[0] = 0;
      continue;
    }
    ...
    msg.data(std::string(frame_data_buffer.begin(), frame_data_buffer.end()))
        .fourcc("h264")
        .width(640)
        .height(480);
    ...
    if (frame_count % 100 == 0) {
      std::cout << "Received " << std::dec << frame_count << " frames"
                << std::endl;
    }
  }
