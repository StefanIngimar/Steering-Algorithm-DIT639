...
def main() -> None:
    ...
    with open(file_name, "rb") as file:
        while True:
            byte = file.read(1)
            if not byte:
                print("Byte not found. Breaking from the main loop")
                break
            buffer += byte
            ...
            if was_envelope_header_consumed:
                if len(buffer) >= expected_bytes:
                    ...
                    frame_ts = envelope.sent.seconds * 1_000_000 + envelope.sent.microseconds
                    ...
                    if envelope.dataType == EnvelopeDataType.GROUND_STEERING_ID.value:
                        ...
                        if image_msg.fourcc.lower() == "h264":
                            ...
                            print(f"Unsupported fourcc value. Expected 'h264' but received '{image_msg.fourcc}'")
                ...
                while len(buffer) >= envelope_header_length:
                    ...
                    if byte0 == 0x0D and byte1 == 0xA4:
                        ...
                        buffer = buffer[1:]
                        print(f"Failed to consume header from Envelope. Expected 0x0D ({0x0D}) and 0xA4 ({0xA4}) as first two bytes but got {byte0} and {byte1} instead")
    ...
    print(f"Found ground steering angles: {read_ground_steering_angles}")
