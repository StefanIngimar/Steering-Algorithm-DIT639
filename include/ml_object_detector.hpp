...
public:
  // Describes the data layout of the model trained to detect and classify cones
  ...
  MlObjectDetector(std::shared_ptr<MlModelRuntime> model_runtime,
...
private:
  ...
  ColorClassifiedCones
  classify_detected_cones(const DetectionResult &detected_objects) const;
  void annotate_frame_with_detected_objects(
      const cv::Mat &frame, const DetectionResult &detected_objects) const;
